...
placeholder = st.empty()
placeholder.write(f"PINN is trained for maturity time = 20, the daily implied volatility found by PINN is {round(vol.numpy(),2)}")
...
if st.button("Show PINN results"):    
    st.subheader(f"Call optime price = {V[0][0] } USD")
...
if st.button('FQI results for option pricing'):
    ...
    st.subheader('---------------------------------\n')
    st.subheader(f'Initial Stock Price: {S0}' )

    st.subheader(f'Strike: {K}')
    st.subheader(f'Maturity: {M}')
    st.subheader(f'The QLBS Call Price 1 :{np.mean(C_QLBS.iloc[:,0])}' ) 
